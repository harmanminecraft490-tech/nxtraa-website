          ...
          return (
            <section key={category} className="border-t border-line py-12">
              <div className="mx-auto w-full max-w-[1500px] px-5 sm:px-8 lg:px-10">
                ...
                <div className="no-scrollbar -mx-5 flex snap-x snap-mandatory gap-2.5 overflow-x-auto px-5 pb-2 sm:mx-0 sm:grid sm:grid-cols-2 sm:gap-8 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-4 lg:gap-10">
                  {categoryProducts.slice(0, 6).map((product) => (
                    <div key={product.id} className="w-[43%] shrink-0 snap-start sm:w-auto sm:shrink">
                      <ProductCard {...product} />
                    </div>
                  ))}
                </div>
